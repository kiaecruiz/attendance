...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    <tr style="font-size:14px; padding-top:100px;">
        ...
        <?php if ($status == 'Low Risk') : ?>
            <td>
                <center>
                    <p style="color:green;  font-weight:500"><?php echo htmlspecialchars($action); ?></p>
                </center>
            </td>
            <?php elseif ($action !== '-') : ?>
            <td>
        ...
        <?php else : ?>
            <td class='lrn-cell'>
                <a data-id='<?= $row['student_id']; ?>' class="button-edit" style="cursor:pointer; text-decoration:none; font-size:10px;">
                    <button>Take an Action</button>
                </a>
            </td>
        <?php endif; ?>

        <td><?php echo htmlspecialchars($result2); ?></td>
    </tr>
    <?php
}
?>

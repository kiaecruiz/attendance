    ...
    if ($_FILES["image"]["error"] == 4) {
        ...
        if (!in_array($imageExtension, $validImageExtension)) {
            ...
            if ($count > 0) {
                ...
                // Insert student data
                $stmt = $conn->prepare("INSERT INTO students (firstName, lastName, student_lrn, gender, image, email, guardian, guardianContact, grade_id, section_id, qr) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                
                if ($stmt) {
                   

                    // Adjust binding based on data types
                    $stmt->bind_param("sssssssiiis", $firstName, $lastName, $student_lrn, $gender, $newImageName, $email, $guardian, $guardianContact, $grade, $section, $generated_code);
                    ...
                    echo "<script> swal('Error', 'Failed to prepare SQL statement.', 'error'); </script>";
                }
            }
        }
    }
